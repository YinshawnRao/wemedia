...
PROJECT = ROOT / "hyperframes" / "projects" / "sichuan-cuisine-hang-to-la-v2"
AUDIO = PROJECT / "assets" / "narration.wav"
EXPORT_DIR = ROOT / "sandbox" / "exports" / "experiment-034"
VIDEO_OUT = EXPORT_DIR / "sichuan-cuisine-hang-to-la-v2-preview.mp4"
CONTACT_OUT = EXPORT_DIR / "sichuan-cuisine-hang-to-la-v2-contact-sheet.jpg"
...
FONTS = {
    ...
    "time": font(FONT_LATIN, 22),
}

CAPTIONS = [
    (0, 4.5, "川菜从夯到拉，主观娱乐版。"),
    (4.5, 9.6, "麻婆豆腐：川菜门面，一勺下去米饭先投降。"),
    (9.7, 14.8, "水煮牛肉：看着凶，其实香麻烫才是核心伤害。"),
    (14.9, 19.6, "回锅肉：锅气一出来，减肥计划当场请假。"),
    (19.7, 23.8, "辣子鸡：找鸡像寻宝，越翻越上头。"),
    (23.9, 28.6, "鱼香肉丝：没鱼但有魂，酸甜咸辣卡得很准。"),
    (28.7, 33.3, "夫妻肺片：红油冷菜天花板，懂的人先夹一筷。"),
    (33.4, 37.7, "口水鸡：名字像玩笑，红油是正经功夫。"),
    (37.8, 42.4, "宫保鸡丁：稳是稳，就是太像菜单默认头像。"),
    (42.5, 47.4, "景区红油套餐：只红不香，只咸不麻，直接拉完。"),
    (47.5, 56.1, "真正的川菜，一城一味，一桌一江湖。你心里的夯菜是哪一道？"),
]
...
CHIPS = [
    ("麻婆豆腐", 0, 38, 4.9, COLORS["signal"], 190),
    ("水煮牛肉", 0, 306, 9.8, COLORS["teal"], 190),
    ("回锅肉", 1, 28, 15.0, COLORS["signal"], 160),
    ("辣子鸡", 1, 244, 19.8, COLORS["teal"], 160),
    ("鱼香肉丝", 1, 462, 24.0, COLORS["yellow"], 190),
    ("夫妻肺片", 2, 36, 28.8, COLORS["signal"], 190),
    ("口水鸡", 2, 306, 33.5, COLORS["teal"], 160),
    ("宫保鸡丁", 3, 44, 37.9, COLORS["yellow"], 190),
    ("景区红油套餐", 4, 42, 42.6, COLORS["signal"], 260),
]
...
def draw_header(draw: ImageDraw.ImageDraw) -> None:
    draw.text((64, 82), "川菜锐评榜", font=FONTS["tag"], fill=COLORS["teal"])
    right = "主观娱乐榜"
...
def draw_board(base: Image.Image, t: float) -> None:
    ...
    for idx, (label, tier) in enumerate(ROWS):
        y = by + row_h * idx
...
def draw_score(draw: ImageDraw.ImageDraw, t: float) -> None:
    thresholds = [4.9, 9.8, 15.0, 19.8, 24.0, 28.8, 33.5, 37.9, 42.6]
    bar_w, gap = 34, 10
...
def draw_closing(base: Image.Image, t: float) -> None:
    ...
    cta = "你心里的夯菜是哪一道？"
    y = 1092 + int((1 - p) * 42)
    draw_text_shadow(d, (80, y), title1, FONTS["close"], COLORS["text"], offset=(0, 4))
...
def write_contact_sheet() -> None:
    samples = [1.8, 5.7, 10.8, 15.8, 20.6, 24.8, 29.7, 34.3, 38.8, 43.5, 50.8]
    ...
    for index, t in enumerate(samples):
        ...
        d.text((x + 8, y + 8), f"{t:04.1f}s", font=FONTS["time"], fill=COLORS["yellow"])
